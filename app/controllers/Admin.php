...
class Admin extends Controller
{
    ...
    public function removePropertyManager($userId)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Log the request
            error_log("RemovePropertyManager called with userId: " . $userId);
            ...
            error_log("Attempting to remove PM with ID: " . $userId);

            // Attempt to remove the manager
            try {
                $result = $this->userModel->removePropertyManager($userId);
                error_log("RemovePropertyManager result: " . ($result ? 'true' : 'false'));
                ...
                if ($result) {
                    ...
                    echo json_encode([
                        'success' => false,
                        'message' => 'Failed to remove Property Manager. Database operation returned false.'
                    ]);
                }
            } catch (Exception $e) {
                error_log("Exception in removePropertyManager: " . $e->getMessage());
                echo json_encode([
                    'success' => false,
                    'message' => 'Error: ' . $e->getMessage()
                ]);
